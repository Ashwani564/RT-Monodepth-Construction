...
Prior work has explored aspects of this challenge in isolation. Yu and Choi \cite{yu2021} demonstrated YOLO-MDE fusion for depth-aware detection but targeted autonomous driving, not construction environments, and neither applied anthropometric scale calibration nor produced proximity-zone safety telemetry. Shen et al.\ \cite{shen2023} introduced scale recovery for MDE specifically on construction scenes, but without real-time object detection or actionable proximity alerts. To the best of our knowledge, no prior system simultaneously addresses all three requirements of a deployable construction safety tool: (i)~real-time fusion of a dedicated MDE model with a construction-domain fine-tuned object detector in a parallelised pipeline, (ii)~automatic metric scale resolution using detected personnel as on-the-fly anthropometric references---requiring no stereo cameras, LiDAR, or pre-placed calibration targets, and (iii)~structured proximity-zone telemetry empirically validated against both benchmark metrics and an automatically graded LLM-supervisor protocol. This paper bridges this gap by introducing a unified framework that synergistically integrates state-of-the-art object detection (YOLOv11) and MDE (RT-MonoDepth). Our primary technical contribution is a novel automatic geometric calibration method that resolves scale ambiguity by using detected personnel as an anthropometric reference, enabling precise, object-specific distance measurements in metres from a single uncalibrated camera. By running these models concurrently in a multi-threaded pipeline, the system transforms standard 2D video into an actionable source of 3D spatial intelligence, creating a deployable safety tool that proactively identifies and logs proximity hazards in real time on construction sites.

In summary, the primary contributions of this paper are:
\begin{enumerate}
    \item \textbf{Real-time MDE--OD fusion for construction safety.} To the best of our knowledge, this is the first system to couple a dedicated real-time monocular depth estimation model (RT-MonoDepth) with a construction-domain fine-tuned object detector (YOLOv11n) in a parallel, multi-threaded pipeline---achieving 22--25~FPS on consumer hardware and 11~FPS on a 2019 NVIDIA Jetson Nano after TensorRT optimisation.
    \item \textbf{Anthropometric scale calibration.} We introduce a hardware-free, on-the-fly scale resolution mechanism that uses detected personnel bounding boxes as anthropometric metric references (assuming 1.7~m adult height) with Exponential Moving Average smoothing, producing stable metric depth estimates without stereo cameras, LiDAR, or pre-placed calibration targets.
    \item \textbf{Structured proximity-zone telemetry.} The system continuously classifies worker--machinery separation into Danger ($<1$~m), Caution ($1$--$3$~m), and Safe ($\geq 3$~m) zones and persists events to auditable CSV logs suitable for post-incident analysis and automated alerting integration.
    \item \textbf{LLM-as-supervisor benchmark.} We introduce and release an automatically graded thirty-question evaluation harness that stress-tests a locally hosted Llama~3.1~8B model against the pipeline's CSV logs, providing the first quantitative capability profile---specific to construction safety telemetry---of an on-device LLM as a natural-language supervisor.
\end{enumerate}

A further contribution---and a direct response to the validation requirements of safety-critical AI---is the \emph{LLM-as-supervisor} evaluation protocol described in contribution~4 above. Rather than restricting validation to numerical accuracy on benchmarks, we treat the system's persistent CSV logs as the ground truth and use a locally hosted Llama~3.1 8B model \cite{dubey2024llama,ollama2024} to answer thirty automatically graded questions. This protocol is motivated by recent work demonstrating both the promise and the limits of vision-language models for hazard identification \cite{adil2025}, RAG-grounded frameworks for automated safety inspection \cite{wang2025}, and comparative analyses of retrieval-augmented versus fine-tuned LLMs for construction knowledge queries \cite{lee2024}. Structured prompting strategies informed by AEC-domain prompt engineering research \cite{samsami2024} further shape the evaluation harness design. The protocol produces a quantitative, reproducible answer to the question of \emph{when an LLM may, and may not, be trusted to summarize safety telemetry}.
